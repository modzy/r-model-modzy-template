...
def run(model_input):
    ...
    with grpc.insecure_channel(f"{HOST}:{port}") as grpc_channel:
        ...
        run_request = RunRequest(inputs=[create_input(model_input)], detect_drift=False, explain=True)
        ...
        run_request_batch = RunRequest(inputs=[create_input(model_input) for _ in range(BATCH_SIZE)], detect_drift=True, explain=False)
...
if __name__ == "__main__":
    ...
    test_inputs = {"input.txt": b"Hello, welcome to Modzy!"}
