...
# import model class from model.R
r = ro.r
r.source('model_lib/src/model.R')
...
def r_list_to_py_dict(r_list):
    """
    Convert an rpy2 <vectorType>Vector to a Python dict
    and validate output contains valid value names
    """
    # TODO: test with legitimate model output with explanation and drift      
    output_min_keys = ["data.result", "data.explanation", "data.drift"]
    r_output_names = list(r_list.names)
    # compare minimum output keys to actual keys in R list 
    check = [name for name in r_output_names if any(key in name for key in output_min_keys)] 
    if len(check) != len(r_output_names):
        raise ValueError("Ouptut list or vector from model.R does not contain required value names. The output must contain at least one copy of each of the following names 'data.result', 'data.explanation', and 'data.drift.'")    
    result = {}
    explanation = {}
    drift = {}
    # loop through each value in r list, check object type, and fill in sub-dictionaries appropriately
    for name in r_list.names:
        output_type = name.split(".")[1]
        key_name = name.split(".")[-1]
        val = r_list.rx(name)[0]
        dict_to_use = result if output_type == "result" else explanation if output_type == "explanation" else drift 
        if isinstance(val, ro.vectors.DataFrame):
            dict_to_use[key_name] = pandas2ri.ri2py_dataframe(val)
        elif isinstance(val, ro.vectors.ListVector):
            dict_to_use[key_name] = r_list_to_py_dict(val)
        elif isinstance(val, ro.vectors.FloatVector) or isinstance(val, ro.vectors.StrVector):
            if len(val) == 1:
                dict_to_use[key_name] = val[0]
            else:
                dict_to_use[key_name] = list(val)
        else: # single value
            dict_to_use[key_name] = val
    
    # aggregate final result into result object with correct dictionary structure
    results = {
        "data": {
            "result": result,
    ...
    def Status(self, request, context):
        ...
        if self.model is None:
            try:
                ...
                self.model = ro.globalenv["r_model_class"]()
                ...
                for i, input_item in enumerate(request.inputs):
                    try:
                        ...
                        raw_outputs = ro.baseenv["$"](self.model, "handle_single_input")(
                            ...
                            ro.vectors.BoolVector([request.explain])
                        )
                        converted_outputs = {"results.json": json.dumps(r_list_to_py_dict(raw_outputs), separators=(",", ":")).encode()}                                                   
                        # TODO: It would probably be useful to have an example of explanation/drift metadata here
                        output_item = create_output_item(f"Processed item {i + 1} as single input.", converted_outputs)
                    except Exception as e:
